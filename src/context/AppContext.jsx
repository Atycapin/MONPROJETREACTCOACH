...
export const AppProvider = ({ children }) => {
  ...
  const [profile, setProfile] = usePersistentState('profile', { 
      onboardingComplete: false, fears: [], goals: [], why: '', name: 'Utilisateur', motto: '' 
  });
  const [userStats, setUserStats] = usePersistentState('userStats', { totalPoints: 0 });
  ...
  const [activePathway, setActivePathway] = usePersistentState('activePathway', null);
  ...
  const addPoints = useCallback((points, reason) => {
    setUserStats(prev => ({ ...prev, totalPoints: prev.totalPoints + points }));
  ...
  const completeOnboarding = useCallback((onboardingData) => {
    ...
    const initialChallenges = onboardingData.goals.flatMap(goal => CONFIG.challenges[goal] || []);
    setChallenges(initialChallenges);
  }, []);
  
  // Ajoute une nouvelle entrée au journal
  const addJournalEntry = useCallback((entry) => {
    setJournal(prev => [{ id: Date.now(), date: new Date(), ...entry }, ...prev]);
  }, []);

  // Marque un rituel comme complété/incomplet pour aujourd'hui
  const toggleRitualCompletion = useCallback((ritualId) => {
    const today = getTodayString();
    const todayLog = ritualLog[today] || [];
    const isCompleted = todayLog.includes(ritualId);
    let newLog = isCompleted ? todayLog.filter(id => id !== ritualId) : [...todayLog, ritualId];
  ...
  const startPathway = useCallback((pathwayId) => {
    ...
    firstWeekTasks.forEach(task => {
      if (task.type === 'ritual' && task.ritual && !userRitualIds.has(task.ritual.id)) {
        ritualsToAdd.push(task.ritual);
      }
    });
    if(ritualsToAdd.length > 0) setRituals(prev => [...prev, ...ritualsToAdd]);

  }, [rituals]);
  
  // Marque une tâche d'un parcours comme complétée
  const completePathwayTask = useCallback((taskId) => {
    if(!activePathway || activePathway.completedTasks.includes(taskId)) return;
      
    setActivePathway(prev => ({ ...prev, completedTasks: [...prev.completedTasks, taskId] }));
